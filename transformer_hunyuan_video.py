...
class HunyuanVideoConditionEmbedding(nn.Module):
    ...
    # def forward(
    #     self, timestep: torch.Tensor, dtype: Optional[torch.dtype] = None
    # ) -> Tuple[torch.Tensor, torch.Tensor]:
    def forward(
        self, timestep: torch.Tensor, guidance: torch.Tensor, dtype: Optional[torch.dtype] = None
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        timesteps_proj = self.time_proj(timestep)
        timesteps_emb = self.timestep_embedder(timesteps_proj.to(dtype=dtype))  # (N, D)
        # pooled_projections = self.text_embedder(pooled_projection)  # Not needed 
        conditioning = timesteps_emb  # + pooled_projections  # No text embedder
        ...
        if self.image_condition_type == "token_replace":
            ...
            # token_replace_emb = token_replace_emb + pooled_projections  # No text embedder
...
class TwoLayerMLP(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        self.ln = Qwen2RMSNorm(input_dim, eps=1e-6)
...
class HunyuanVideoTransformer3DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin, CacheMixin):
    ...
    def __init__(
        ...
        # pooled_projection_dim: int = 768,  # Not needed
        ...
        assert image_condition_type is None
        ...
        self.context_embedder = HunyuanVideoTokenRefiner(
            4096, num_attention_heads, attention_head_dim, num_layers=num_refiner_layers
        )

        # qwen text in
        self.qwen_project_in = TwoLayerMLP(
            text_embed_dim, text_embed_dim*4, 4096
        )

        #  Remove guidance_embeds
        # self.time_text_embed = HunyuanVideoConditionEmbedding(
        #     inner_dim, image_condition_type
        # )
        self.time_text_embed = HunyuanVideoConditionEmbedding(
            inner_dim, guidance_embeds, image_condition_type
        )
    ...
    def forward(
        ...
        timestep: torch.LongTensor,
        # input_info: Optional[Dict[str, Any]],  # Exp 7_3
        encoder_hidden_states: torch.Tensor,
        encoder_attention_mask: torch.Tensor,
        video_voxel_mask: Optional[torch.Tensor] = None,  # expect to be [B,1,F,H,W]
        ...
        if video_voxel_mask is not None:
            assert (
                    video_voxel_mask.shape
                    == (hidden_states.size(0), 1, *hidden_states.shape[2:])
            ), f"video_voxel_mask must be [B,1,F,H,W], got {tuple(video_voxel_mask.shape)} and {tuple(hidden_states.shape)}"
        ...
        temb, token_replace_emb = self.time_text_embed(timestep, guidance, dtype=hidden_states.dtype)
        # temb, token_replace_emb = self.time_text_embed(timestep, dtype=hidden_states.dtype)

        hidden_states = self.x_embedder(hidden_states)

        # Use mlp + original refiner
        encoder_hidden_states = self.qwen_project_in(encoder_hidden_states)
        ...
        attn = torch.ones(batch_size, sequence_length, device=hidden_states.device, dtype=torch.bool)

        # (A) apply text mask
        effective_ctx = encoder_attention_mask.sum(dim=1, dtype=torch.int)
        effective_seq = latent_sequence_length + effective_ctx
        indices = torch.arange(sequence_length, device=hidden_states.device).unsqueeze(0)
        attn = attn.masked_fill(indices >= effective_seq.unsqueeze(1), False)

        # (B) apply video token mask, if provided
        if video_voxel_mask is not None:
            def voxel_mask_to_token_mask(voxel_mask: torch.Tensor, p_t: int, p: int) -> torch.Tensor:
                """
                voxel_mask: [B, 1, F, H, W] with 1 for real content and 0 for padding.
                Returns:    [B, N_latent] bool mask for tokens after 3D patching (Conv3d stride=(p_t,p,p)).
                """
                # pool with the SAME kernel & stride as your patch embedder so each token corresponds to a pooled cell
                pooled = F.avg_pool3d(voxel_mask.float(), kernel_size=(p_t, p, p), stride=(p_t, p, p))
                assert torch.all((pooled == 0) | (pooled == 1))
                token_mask = pooled.flatten(1).bool()       # [B, N_latent]
                return token_mask
            B = hidden_states.size(0)
            assert video_voxel_mask.ndim == 5 and video_voxel_mask.size(0) == B and video_voxel_mask.size(1) == 1, f"video_voxel_mask must be [B,1,F,H,W], got {tuple(video_voxel_mask.shape)}"
            video_token_mask = voxel_mask_to_token_mask(video_voxel_mask, p_t, p)  # [B, N_latent] bool
            assert video_token_mask.shape == (B, latent_sequence_length),  f"token mask length {video_token_mask.shape[1]} != latent {latent_sequence_length}"
            attn[:, :latent_sequence_length] = video_token_mask

        # expand for SDPA: [B, 1, 1, N_total]
        # attention_mask = attn.unsqueeze(1).unsqueeze(1)
        attention_mask = None
        ...
        for block in self.transformer_blocks:
            if self.training and torch.is_grad_enabled() and self.gradient_checkpointing:
                # log_to_rank0("HunyuanVideo transformer_blocks use gradient_checkpointing")
        ...
        for block in self.single_transformer_blocks:
            if self.training and torch.is_grad_enabled() and self.gradient_checkpointing:
                # log_to_rank0("HunyuanVideo single_transformer_blocks use gradient_checkpointing")
                hidden_states, encoder_hidden_states = torch.utils.checkpoint.checkpoint(
                    create_custom_forward(block),
                    hidden_states,
                    encoder_hidden_states,
